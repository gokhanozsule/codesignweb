...
export default function Blop(props) {
  ...
  useFrame(({ clock }) => {
    ...
    group.current.position.x = THREE.MathUtils.lerp(group.current.position.x, 0.1, 0.05)
    group.current.position.z = THREE.MathUtils.lerp(group.current.position.z, 0.1, 0.03)
  })
  return (
    <group {...props} dispose={null}>
      <group ref={group}>
        <mesh geometry={nodes.geo.geometry} castShadow receiveShadow>
          <MeshDistortMaterial color="#516754"  roughness={1} metalness={0.5} factor={15} speed={5} />
        </mesh>
      ...
      <group position={[1.25, -0.5, 0]} scale={0.5}>
        <Text position={[-2.5,4.5,-1]} fontsize={0.03} lineHeight={0.5} letterSpacing={-0.05} >
          Animation
